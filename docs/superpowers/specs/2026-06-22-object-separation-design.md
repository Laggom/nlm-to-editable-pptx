...
- **1순위**: 인터넷에서 구한 **실제 NotebookLM 통이미지 PPT/PDF** (개인 산출물이라 공개 다운로드가 드물 수 있음 — GitHub 호스팅 샘플 우선).
- **폴백**: NBLM2PPTX 데모 자산(before 이미지), 또는 통이미지 슬라이드 프록시 생성.
- (SK 내부 덱은 기밀이므로 공개 비교/배포에는 쓰지 않음.)
