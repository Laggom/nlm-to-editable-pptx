...
각 쌍의 **왼쪽이 NotebookLM 원본**(통이미지, 편집 불가), **오른쪽이 변환 결과**(원본 글자를
제거한 배경 위에 편집 가능한 텍스트). 한국어·수식·표가 모두 편집 가능한 텍스트로 재구성됩니다.
...
- 변환 결과 샘플 파일: [`examples/sample_output.pptx`](examples/sample_output.pptx)
